#
#   simulation_basic_grids_full_record.py
#
#   Main simulation run: grid fields with theta input and all the inhibition
#   (for gamma) and place input. This simulation run stores membrane potentials
#   and syn. currents for the duration of whole simulation.
#
#   This will eat a lot of memory and produce a BIG file!
#
...
stateMon_e          = StateMonitor(ei_net.E_pop, 'vm',     record = state_record_e, clock=stateRecClock)
stateMon_i          = StateMonitor(ei_net.I_pop, 'vm',     record = state_record_i, clock=stateRecClock)
stateMon_Iclamp_e   = StateMonitor(ei_net.E_pop, 'Iclamp', record = state_record_e, clock=stateRecClock)
stateMon_Iclamp_i   = StateMonitor(ei_net.I_pop, 'Iclamp', record = state_record_i, clock=stateRecClock)
stateMon_Iext_e     = StateMonitor(ei_net.E_pop, 'Iext',   record = state_record_e, clock=stateRecClock)
stateMon_Iext_i     = StateMonitor(ei_net.I_pop, 'Iext',   record = state_record_i, clock=stateRecClock)
