...
###############################################################################
# E-surround parameters
ESurrParams = deepcopy(defaultParams)

###############################################################################
# I-surround parameters
ISurrParams = deepcopy(defaultParams)
ISurrParams.update(
    AMPA_gaussian   = 1,        # bool
    pAMPA_mu        = 1.2/0.6,  # normalized
    g_AMPA_total    = 4500,     # nS
    g_GABA_total    = 400,      # nS
    g_uni_GABA_frac = 0.3125    # nS
)
...
def decideParams(simType):
    if (simType == 'E_surround'):
        return deepcopy(ESurrParams)
    elif (simType == 'I_surround'):
        return deepcopy(ISurrParams)
    else:
        raise ValueError("Unknown simulation type: {0}".format(simType))
