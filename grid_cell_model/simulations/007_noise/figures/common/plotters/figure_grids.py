...
class GridSweepsPlotter(SweepPlotter):
    ...
    NTrials = 3
    trial_num_list = np.arange(self.NTrials)
    cmap = 'jet'
    ...
    def plot(self, *args, **kwargs):
        ...
        #example_idx = myc['example_idx']

        for ns_idx, noise_sigma in enumerate(ps.noise_sigmas):
            fig, ax = self.get_fig(), self.get_ax()
            kw = dict(cbar=False)
            if ns_idx == 0:
                kw['cbar'] = True
            if ns_idx != 0:
            ...
            sweeps.plotGridTrial(ps.grids[ns_idx], varList, ds.iterList,
                    ...
                    trialNumList=self.trial_num_list,
                    r=self.exampleIdx[ns_idx][0], c=self.exampleIdx[ns_idx][1],
                    ax=ax,
                    cbar_kw=cbar_kw,
                    cmap=self.cmap,
                    vmin=vmin, vmax=vmax,
