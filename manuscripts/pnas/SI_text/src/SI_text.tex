...
\newcommand{\Gmax    }{\ssc{G}      {max}{}}
\newcommand{\GmaxAMPA}{\ssc{G}      {max}{\scriptsize{AMPA}}}
\newcommand{\Ginh    }{\ssc{G}      {inh}{}}
...
\begin{center}
{\large\bfseries
Noise promotes independent control of gamma oscillations and grid firing by
regulation of recurrent inhibitory and excitatory synaptic connections -
...
\section*{SI Methods} \label{methods}
...
The FS interneurons do not posess AHP, but instead contain a simple adaptation
...
neuron $i$ were modeled as exponentials with user defined time constants (see
table \ref{tab:params_synapses} for the parameter values):
\begin{align}
...
tens of ms \citep{JONES:1993je}. In this work, NMDA synapses are thus
represented by an exponentially decaying conductance ($\gNMDA$), with a 100 ms
time constant (Table \ref{tab:params_synapses}).
...
cells ($\Iplace$).  The description of the parameters in equation
\eqref{eq:Itheta} can be found in table \ref{tab:neuron_params}.

While $\Iconst$ and $\Itheta$ are simple functions of time, the velocity
modulated current and place cell current deserve special attention and
therefore will be described in separate sections. The velocity modulated
current is described in section \ref{sec:Ivel} and the place cell input current
in section \ref{sec:place_cells}.
\begin{table}
    \centering
    \begin{tabular}{l | l || l | l}
        ...
        $\gAHPmax$ & AHP maximal value         & $\gadinc$  & Adaptation conductance incrase \\
    ...
    in the simulations, refer to tables
    \ref{tab:params_E}-\ref{tab:params_ext_inputs}.}
...
Recent experimental evidence shows that stellate cells synapse onto each other
...
Feedback connections between interneurons are also missing, however this type
of architecture is sufficient to generate theta nested gamma oscillations even
in their absence \citep{Pastoll:2013ff} and therefore addition of
$I{\rightarrow}I$ connections would not qualitatively change the results.
...
takes the boundaries of the torus into account, and $\mathbf{C}$ is the
...
velocity of the bump onto the animat velocity
...
function
\begin{equation}
    \wGABAA^{ij} = \Ginh \exp\left(\frac{-d(i, j, 0, 0)^2}{2 \sigmasub{inh}^2}\right),
    \label{eq:wGABA}
\end{equation}
...
in eq. \eqref{eq:dijce}.

%Figure \ref{fig:pref_directions} shows
%the distribution of preferred directions on a smaller patch of E cells on the
%twisted torus.
%\begin{figure}[tb]
%    \begin{center}
%        \includegraphics[width=0.35\textwidth]{fig/pref_directions}
%    \end{center}
%    \caption{\figtitle{Distribution of preferred directions of neurons on a
%    patch of the neural sheet.} All E neurons on the neural sheet were assigned
%    a preferred direction of a shift of their outgoing synaptic profile. Each
%    arrow illustrates the direction of $\mathbf{e}_p$, the unit vector from eq.
%    \eqref{eq:dijce}. Therefore, there are four sub-populations of E cells,
%    determined by their preferred directional vectors. Red color highlights the
%    sub-population with their outgoing synaptic profiles shifted upwards.}
%    \label{fig:pref_directions}
%\end{figure}
...
might drift over time. The simulations of grid firing fields (Figure 1) and
simulations that explored the controllability of the network by place cell
input (Figure \ref{fig:S_pc_effectivity}) comprised a separate population of
cells with place-like firing fields (in all other simulations the input was
de-activated). Inputs from these cells opposed drift of the bump attractor.
...
0.5 nS and $\sigma_\text{PC}$ = 7 cm. Connections from place cell were modelled
...
attractor position so that the phase of grid firing fields match the positions
...
used the maximal conductance of AMPA ($\Gmax$, eq. \ref{eq:wAMPA})
as the $\gE$ value, and the maximal conductance of GABA synapses
($\Ginh$, eq. \ref{eq:wGABA}) as the $\gI$ value in all the
parameter exploration plots. Note that NMDA conductances are not counted
towards $\gE$\ and that there is no recurrent excitation nor inhibition
within the excitatory and inhibitory populations themselves.
...
the minum of the values at 60 and 120 degrees rotation.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Estimating gain of the velocity-dependent inputs}

In order to estimate the precision of velocity integration in a continous
attractor, we have performed shorter simulations in which a constant velocity
input (in the X direction) has been injected into E cells for a period of 10
...
\begin{enumerate}
    \item Estimate the range of bump speeds that need to be covered (figure
        \ref{fig:velocity_histograms}).
        \begin{equation}
        s_{\text{bump}}^i =
        ...
        without error (figure \ref{fig:velocity_histograms}B).
    \item Pick a specified percentile from this distribution, i.e. the maximal
        speed of the bump, in order to account for the specified fraction of
        animal velocities, set this as $s_{\text{max}}$. The range of target
        bump speeds will be $<0, s_{\text{max}}>$.
    \item For each $I_{\text{vel}} \in \{0, 10, .., 100\}$ pA, estimate the
        bump speed by tracking its position on the neural sheet, using
        population estimator. Repeat this step 10 times. This step acquires
        data for estimating the slope of bump speed vs. injected velocity
        current relationship.
    \item For each $I_{\text{vel}}^{\text{max}} \in \{10, 20, .., 100\}$ pA,
...
number of shorter simulations with certain data analysis steps.  The analysis
of the shorter simulation runs determines how much current needs to be injected
in order for the bump of activity to track the simulated movement of an animal.
This determines the gain of the velocity input current in order to produce grid
fields with a specified spacing between the peaks in the firing field.  The
result is a single number in units of neurons/s/pA, which determines the speed
of the bump as a function of injected velocity input. The spacing between the
...
Each of the runs is repeated 3 times. These simulations use the estimated
velocity response gain in order to calibrate the spacing between the grid
firing fields.  After the simulation is finished, a neuron in the bottom-left
corner of the torus is selected for analysis. For this cell the gridness score
of its firing field is computed. The
reasoning behind choosing only a single cell to estimate the gridness score is
as follows. The grid firing fields in the network are a result of coordination
of activity of the network as a whole. If the network forms a bump attractor
that is able to accurately track animal movement, all cells in the network will
have grid-like firing fields that differ only in their phases. On the other
hand, if the bump attractor does not form, is unstable, or does not accurately
track the position of the animal, the gridness score of all cells will be low.
Thus, the firing field of a single cell in the network represents grid field
computation in the network as a whole.  Moreover, this cell can be selected
arbitrarily.
...
excitatory cells in the model. Thus, on each run, different cells could be
...
randomly selected excitatory cells, by clamping their membrane potential at
...
\begin{table}
    ...
    \begin{tabular}{| c | c | c |}
        ...
        \hline
    \end{tabular}
    \caption{Parameter values for synapes.}
...
\begin{figure}[h!]
    \caption{Examples of spatial firing fields. (A-L) Top: Gridness score in
    ...
    Noise level use in each set of simulations is shown by
    $\sigma_{\text{noise}}$. Figure continues on next pages.}
    \label{fig:S_grid_examples}
\end{figure}
...
\begin{figure}[ht!]
    ...
    networks with the highest simulated noise level (C). The strength of the
    ...
    network and detecting their first local maxima. The correlation value at
    the first local maximum is plotted on the abscissa. Colour coding
    determines the values of $\gE$ and $\gI$, as shown in the 2D colorbar.}
    \label{fig:noise_scatter_gamma_grids}
\end{figure}

\begin{figure}[ht!]
    \caption{Scatter plots of gridness score vs. detected oscillation frquency.
    (A-C) The plots show relationships between grid field computations
    (gridness score) and the frquency of gamma oscillations for deterministic
    networks (A), networks with moderate noise (B) and networks with the
    highest simulated noise level (C). The frequency of the oscillation was
    obtained by computing autocorrelation functions of inhibitory currents
    impinging onto 25 randomly selected E cells in the network, detecting their
    first local maxima, and averaging over all cells.  The time lag at the
    first local maximum yielded the frequency of the oscillation, which is
    plotted on the abscissa. Colour coding determines the values of $\gE$ and
    $\gI$, as shown in the 2D colorbar.}
    \label{fig:noise_scatter_gamma_grids_freq}
\end{figure}
...
\begin{figure}[ht!]
    ...
    rates of E and I cells are colored red and blue, respectively. The position
    in the parameter space is highlighted by an arrow. Figure continues on next
    pages.}
\end{figure}
...
\begin{figure}[ht!]
    ...
    (Figure 1). (B) Relationship between gridness score and firing frequency of
    E cells.}
\end{figure}
...
\begin{figure}[ht!]
    ...
    used to fit a linear relationship using an stimation procedure outlined
    in SI methods (black line). (D-F) Slope of the estimated velocity gain of
    ...
    for the estimated linear relationships.}
\end{figure}
...
\begin{figure}[ht!]
    ...
    with an equal mean amplitude. (A-C) Amplitude of detected gamma
    oscillations (SI methods) in deterministic networks (A), networks with an
    intermediate level of noise (B) and in networks with the highest simulated
    level of noise (C). (D-F) Same as for (A-C) but the plots show the
    probability of the network forming a bump attractor (SI methods). Each
    point is a an average of five simulation runs. All data are from the same
    simulation set.}
\end{figure}
\end{document}
