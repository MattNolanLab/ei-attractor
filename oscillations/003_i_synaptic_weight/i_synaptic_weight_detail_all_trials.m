% Create population detailed plot for all trials, specified by wi
% value
close all;
...
wi_all = [60 65 70] * 1e-3 / 1000;
find_eps = 1e-9;

for wi = wi_all
    par_it = 1;
    while par_it <= nParam
        if abs(results(par_it, 1).opt.wi - wi) < find_eps
            break;
        end;
        par_it = par_it + 1;
    end
    if par_it > nParam
        warning(sprintf('Could not find wi = %f mV', wi*1000));
        continue;
    end;
    ...
    for trial_it = 1:nTrials
        ...
        print('-depsc2', sprintf('%s/e_synaptic_weight_population_detail_wi_%.3fmV_trial_%.3d.eps', outputDir, opt.wi*1000, trial_it));
    end
end