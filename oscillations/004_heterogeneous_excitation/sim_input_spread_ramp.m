...
% excitation current
%
% The input excitation is a ramping current
...
for spread = opt.input_spread_vec
    ...
    % spread function
    opt.Ie = zeros(opt.Ne, 1) + opt.Ie_0;
    opt.Ii = zeros(opt.Ni, 1) + opt.Ii_0;
    Ie_max = gaussianSpread(opt.D*rand(opt.Ne, 1), opt.input_spread, opt.Ie_max);
    Ii_max = gaussianSpread(opt.D*rand(opt.Ni, 1), opt.input_spread, opt.Ii_max);
...
clear tmpresults;
save('-v7.3', sprintf('004_gaussian_spread_ramp_%s.mat', datestr(now, 'yyyy-mm-dd_HH-MM-SS')));
