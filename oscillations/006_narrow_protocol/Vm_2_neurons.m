...
parfor it1 = 1:size(res.Vmon.gi, 1)
    for it2 = it1+1:size(res.Vmon.gi, 1)
        figure('Position', [0 0 1600 800], 'Visible', 'off');
        
        gi_correlations_EE(res, it1, it2, ti_start, ti_end, win_len, plot_opt);

        set(gcf,'PaperPositionMode','auto');    
        print('-depsc2', sprintf('%s/%s_narrow_2Vm_exc_trial_%.3d_nid_%.3d_%.3d.eps', ...
                outputDir, outputNum, trial_it, res.opt.Emon_i(it1), res.opt.Emon_i(it2)));    
    end
end
