...
# Principal cell settings
optParser.add_option("--prin-Cm",  type="float", dest="prin_Cm")
optParser.add_option("--prin-gL",  type="float", dest="prin_gL")
optParser.add_option("--prin-VL",  type="float", dest="prin_VL")
optParser.add_option("--prin-VE",  type="float", dest="prin_VE")
optParser.add_option("--prin-VI",  type="float", dest="prin_VI")
optParser.add_option("--prin-Vth", type="float", dest="prin_Vth")

# Interneurons settings
optParser.add_option("--int-Cm",  type="float", dest="int_Cm")
optParser.add_option("--int-gL",  type="float", dest="int_gL")
optParser.add_option("--int-VL",  type="float", dest="int_VL")
optParser.add_option("--int-VE",  type="float", dest="int_VE")
optParser.add_option("--int_VI",  type="float", dest="int_VI")
optParser.add_option("--int-Vth", type="float", dest="prin_Vth")

# Synaptic parameters
optParser.add_option("--syn-sigma", type="float", dest="syn_sigma")
optParser.add_option("--syn-gEE_bar", type="float", dest="syn_gEE_bar")
optParser.add_option("--syn_gIE_bar", type="float", dest="syn_gIE_bar")

optParser.add_option("--tau-GABA", type="float", dest="tau_GABA", help="GABA_A synaptic time constant (ms)")
optParser.add_option("--tau-AMPA", type="float", dest="tau_AMPA", help="AMPA time constant (ms)")
optParser.add_option("--tau-NMDA", type="float", dest="tau_NMDA", help="NMDA time constant (ms)");

# Network level settings
optParser.add_option("--sheet-size", type="int", dest="sheet_size")

# Afferent excitatory input parameters
optParser.add_option("--aff-mean", type="float", dest="aff_mean");
optParser.add_option("--aff-sigma", type="float", dest="aff_sigma");

# Simiulation timing settings
optParser.add_option("--sim-dt", type=float, dest="sim_dt", help="Simulation time step (ms)")
optParser.add_option("--sim-time", type="float", dest="sim_time", help="Total simulation time [seconds]")
optParser.add_option('--update-interval', type="float", dest="update_interval", help="Duration between simulation status printouts")

# Other settings

#optParser.add_option("-w", "--write-data", action="store_true", dest="write_data", default=False)
#optParser.add_option("-n", "--job-num", type="int", default=-1, dest="job_num", help="Use argument of this option to specify the output file name number, instead of using time")
